...
Examples
--------
Phase portrait of a regulator.
$ (cd .. && make regulator_test)
$ ../regulator_test >regulator.dat
$ uv run python plot_phase_portrait.py regulator.dat
...
def load(argv: list[str]) -> tuple[TaggedData, list[str]]:
    dataset = TaggedData(argv[1])
    tags = []
    if len(argv) == 2:
        # No tags specified.
...
def plot(dataset: TaggedData, tags: list[str]) -> None:
    # Prepare figure.
    _, ax = plt.subplots()
